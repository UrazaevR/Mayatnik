...
from PyQt6.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QCheckBox
...
class Window(QWidget):
    def __init__(self):
        ...
        self.canvas.axes.set_xlabel('Амплитуда, град')
        self.canvas.axes.set_ylabel('Период, с')
    ...
    def _update_plot(self, x1, y1, x2, y2):
        ...
        self.canvas2.axes.set_ylim(0, 3)
        self.canvas.axes.set_xlim(0, round(float(self.END_TIME_edit.text()) / 100, 1) * 100)
    ...
    def draw_plot(self):
        ...
        self.calc_thread.finished.connect(lambda *x: self.calculate_but.setEnabled(True) == self.stop_but.setDisabled(True))
